...
interface ConfirmJoinRoomWindowProps {
  room: RoomInterface;
  onClose: () => void;
  onCloseButt: () => void;
}

const ConfirmJoinRoomWindow: React.FC<ConfirmJoinRoomWindowProps> = ({
  room,
  onClose,
  onCloseButt,
}) => {
  ...
  const buttonHandle = async (event: React.MouseEvent<HTMLButtonElement>) => {
    if (roomInfor && roomInfor?.participants && accountDetails) {
      let isParticipantExisting = roomInfor.participants.some((participant) => {
        return participant.email === accountDetails.email;
      });

      if (isParticipantExisting) {
        console.log("Bạn đã tham gia cuộc đấu giá này.");
        notify("Bạn đã tham gia cuộc đấu giá này.", "error");
      } else {
...
export default ConfirmJoinRoomWindow;
