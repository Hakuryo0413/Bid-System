...
import { getAllHistories, getRoomByAccount } from "../../../features/axios/api/room/RoomDetails";
import { get } from "lodash";
import { updateRoom } from "../../../features/axios/api/room/UpdateRoom";
import { useForm } from "react-hook-form";
function ConfirmLog({ setOpen, selectedHuy, user }: { setOpen: (value: boolean) => void; selectedHuy?: RoomInterface; user?: userInterface }) {
        ...
        try {
            ...
            if (selectedHuy) {
                ...
                if (participants && user && user.email) {
                    let newParticipants = participants.filter((item) => item.email !== user.email);
...
function SimWaitingList() {
    ...
    return (
        <div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {roomList.map((room, index) => (
                            <tr key={index}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm text-gray-900">{String(room.start_at)}</div>
                                </td>
            ...
            {open && <ConfirmLog setOpen={setOpen} selectedHuy={selectedHuy} user={user} />}        </div>
